...
def test_read_flow():
    ...
    # --- 4a: Always-mask file (.env) → redirect to temp ---
    tmpdir = tempfile.mkdtemp()
    ...
    # Test offset/limit passthrough on always-mask file
    ...
    # --- 4b: Non-always-mask file → warn only, no redirect ---
    config_file = os.path.join(tmpdir, "config.yml")
    with open(config_file, "w") as f:
        f.write("db: postgres://admin:secret@db:5432/app\nname: myapp\n")

    stdout3, _, code3 = run_hook("sensitive-data-mask.py", {
        "tool_name": "Read",
        "tool_input": {"file_path": config_file},
        "session_id": "test-read-flow",
    })

    assert_eq(code3, 0, "Warn-only exits 0")
    if stdout3:
        out3 = json.loads(stdout3)
        hook3 = out3.get("hookSpecificOutput", {})
        assert_true(hook3.get("updatedInput") is None,
                    "Warn-only: no updatedInput (preserves edit)")
        assert_in("[SECURITY]", hook3.get("additionalContext", ""),
                  "Warn-only: has security warning")
...
def test_settings_integrity():
    print("\n[10] Settings.json — Hook Registration Integrity")
    ...
    # PreToolUse preserves secret-scanner
    scanner_entry = [e for e in pre
                     if "secret-scanner.py" in str(e)]
    assert_true(len(scanner_entry) > 0, "PreToolUse preserves secret-scanner")
    ...
    # PostToolUse preserves code-quality
    quality_entry = [e for e in post
                     if "code-quality.py" in str(e)]
    assert_true(len(quality_entry) > 0,
                "PostToolUse preserves code-quality")
    ...
    assert_true("mcpServers" in settings, "mcpServers preserved")
    assert_true("enabledPlugins" in settings, "enabledPlugins preserved")
    assert_true("statusLine" in settings, "statusLine preserved")
